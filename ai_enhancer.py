...
def process_admin_conversational_assistant(
    ...
    prompt_text = (
        ...
        "- Schedule a range of old messages to auto-deliver every week during a class's actual time slot "
        "(spread 2-5 min apart, capped to the session duration, resuming where it left off each session "
        "until the whole range is delivered) - the admin never has to trigger this manually: "
        "`<ACTION>{\"type\": \"autodeliver\", \"source_id\": <int>, \"dest_id\": <int>, \"start_id\": <int>, "
        "\"end_id\": <int>, \"days\": \"Mon,Wed,Fri\", \"start_time\": \"HH:MM (24h UTC)\", "
        "\"duration_minutes\": <int>, \"name\": \"<optional label>\"}</ACTION>`\n"
        "- Cancel a weekly auto-delivery schedule: `<ACTION>{\"type\": \"stopautodeliver\", \"name\": \"<label>\"}</ACTION>`\n\n"
        "IMPORTANT ABOUT SCHEDULING: autodeliver's start_time is always UTC. If the admin gives a local time "
        "and their timezone/offset is known or stated, convert it to UTC yourself before emitting the ACTION. "
        "If you don't know their offset, ask once rather than guessing.\n\n"
